...
#[test]
fn test_csharp_generated_files_contain_warning_header() -> Result<()> {
    let temp_dir = tempdir()?;
    let grammar_path = temp_dir.path().join("header_comment.par");

    fs::write(
        &grammar_path,
        r#"%start HeaderComment
%title "Header comment regression"
...
"#,
    ...
    let parser_path = temp_dir.path().join("HeaderCommentParser.cs");
    let actions_path = temp_dir.path().join("IHeaderCommentActions.cs");
    ...
    let actions_source = fs::read_to_string(actions_path)?;

    for source in [&parser_source, &actions_source] {
        assert!(
            source.contains("This file was generated by parol.")
                && source.contains("Do not edit this file manually.")
                && source.contains("Changes will be overwritten on the next build."),
            "Expected generated C# source to contain warning header comment"
        );
    }
