...
type TransferFunction = Rc<dyn Fn(Rc<ResultMap>, Rc<RefCell<FollowSet>>) -> DomainType + 'static>;
...
///
/// ## Parameters
///   * `es: Rc<EquationSystem>` - An `Rc` of an `EquationSystem` struct.
...
///     It is wrapped in an `Rc<RefCell>` to make it write accessible from multiple threads.
/// ## Return Value
/// The `StepFunction` returns a `ResultMap` struct that was extended in each iteration step.
type StepFunction = Rc<
    dyn Fn(
        Rc<EquationSystem>,
        Rc<ResultMap>,
        Rc<HashMap<Pos, usize>>,
        Rc<RefCell<FollowSet>>,
    ) -> ResultMap,
>;
...
pub fn follow_k(
    ...
    let equation_system = Rc::new(equation_system);

    let step_function: StepFunction = Rc::new(
        move |es: Rc<EquationSystem>,
              result_map: Rc<ResultMap>,
            ...
            result_map.iter().for_each(|(pos, _)| {
                ...
                let pos_result = es[pos](Rc::clone(&result_map), Rc::clone(&non_terminal_results));
    ...
    N: NonTerminalIndexFn + Send + Sync + 'static,
{
    ...
    for (part_index, (symbol_index, symbol_string)) in parts.iter().enumerate() {
        if let Symbol::N(..) = &symbol_string.0[0] {
            let mut result_function: TransferFunction = Rc::new(move |_, _| {
                DomainTypeBuilder::new()
                    .k(args.k)
            ...
            for (_, symbol_string) in parts.iter().skip(part_index + 1) {
                ...
                match symbol {
                    Symbol::T(_) => {
                        ...
                        result_function =
                            Rc::new(move |result_map: Rc<ResultMap>, non_terminal_results| {
